...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!wallet?.address || !isAddress(wallet.address)) {
      return Response.json({ error: "no embedded wallet for user" }, { status: 500 });
    }
    ...
    if (relayerKey) {
      ...
      if (balance < parseEther("0.3")) {
        ...
        const wc = createWalletClient({ account: relayer, chain: aeneid, transport: http() });
        const hash = await wc.sendTransaction({ to: heir, value: parseEther("0.5"), account: relayer, chain: aeneid });
    ...
    return Response.json({ error: "provision failed", detail: String(e) }, { status: 500 });
  }
}
