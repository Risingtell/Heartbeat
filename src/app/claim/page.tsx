        ...
        <div className="mx-auto max-w-2xl px-5 py-14 w-full space-y-6 animate-fade-up">
          ...
          <div className="text-center">
            <p className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] font-medium text-accent-deep">
              <span className="h-1.5 w-1.5 rounded-full bg-accent animate-heartbeat" /> Beneficiary
            </p>
            <h1 className="mt-3 text-3xl sm:text-4xl font-semibold tracking-tight leading-tight">
              Recover a <span className="font-display italic font-normal text-accent-deep">vault.</span>
            </h1>
            <p className="mt-2 text-foreground-soft max-w-md mx-auto text-sm">
              Open your benefactor&apos;s claim link — or enter the details below — sign in with the email they registered, and unlock.
            </p>
          </div>
          ...
          {!address && (
            <div className="rounded-2xl border border-border bg-surface p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-accent-glow transition-colors">
              <div>
                <p className="text-[11px] uppercase tracking-[0.16em] text-accent-deep font-medium">Step 1</p>
                <div className="mt-1 font-semibold">Sign in</div>
                <p className="text-sm text-muted mt-1">
                  Use the <span className="font-medium text-foreground">email</span> your benefactor registered for you — we email you a code, no wallet or seed phrase needed. (Or connect your own wallet.)
                </p>
              </div>
          ...
          <div className="rounded-2xl border border-border bg-surface p-6 space-y-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.16em] text-accent-deep font-medium">{address ? "" : "Step 2"}</p>
              <p className={`font-semibold ${address ? "" : "mt-1"}`}>Vault details</p>
              <p className="text-xs text-muted mt-1">Filled in automatically when you open a claim link.</p>
            </div>
            <label className="block">
              <span className="text-sm font-medium">Owner address</span>
              <input
                value={owner} onChange={(e) => setOwner(e.target.value)} placeholder="0x…"
            ...
            <label className="block">
              ...
              <input
                value={uuid} onChange={(e) => setUuid(e.target.value)} placeholder="e.g. 4077" inputMode="numeric"
                className="field-input mt-2 font-mono text-sm"
              />
            </label>
          </div>
            ...
            <div className="relative rounded-3xl border border-border bg-surface overflow-hidden">
              ...
              <div className="px-6 sm:px-10 py-10">
                <div className="flex items-center gap-4">
                  <span className={`heart-halo text-4xl leading-none ${view.claimable ? "text-warm" : "text-accent animate-heartbeat"}`}>♥</span>
                  <div>
                    <p
                      className="text-[11px] uppercase tracking-[0.18em] font-medium"
                      style={{ color: view.claimable ? "var(--warm)" : "var(--accent-deep)" }}
                    >
                    ...
                    <p className="mt-0.5 text-lg font-semibold">
                      {view.claimable
                        ? "Available to unlock"
                        : <>Unlocks in <span className="font-display italic font-normal text-accent-deep">{humanDuration(secondsLeft)}</span></>}
                    </p>
                ...
                <dl className="mt-7 grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                  <div className="rounded-xl bg-surface-2 border border-border-soft px-3.5 py-3">
                    <dt className="text-[11px] uppercase tracking-wider text-muted">Beneficiary</dt>
                    <dd className="mt-1 font-medium font-mono">{shortAddr(view.heir)}</dd>
                  </div>
                  <div className="rounded-xl bg-surface-2 border border-border-soft px-3.5 py-3">
                    <dt className="text-[11px] uppercase tracking-wider text-muted">Inactivity window</dt>
                    <dd className="mt-1 font-medium">{humanDuration(Number(view.period))}</dd>
                  </div>
                  {view.guardianThreshold > 0 && (
                    <div className="rounded-xl bg-surface-2 border border-border-soft px-3.5 py-3">
                      <dt className="text-[11px] uppercase tracking-wider text-muted">Guardian attestations</dt>
                      <dd className="mt-1 font-medium">{view.attestations}/{view.guardianThreshold}</dd>
                    </div>
                  )}
                </dl>
                ...
                {!address ? (
                  <div className="mt-7">
                    <p className="text-sm text-muted mb-3">Sign in with the email your benefactor used (or your own wallet) to unlock.</p>
                    <LoginButton label="Sign in to unlock" />
                  </div>
                ) : isHeir ? (
                  <button
                    onClick={unlock} disabled={!view.claimable || unlocking || uuid === ""}
                  ...
                  >
                    {unlocking
                      ...
                      : view.claimable
                        ? <span className="inline-flex items-center gap-2">Unlock the secret <span className="transition-transform inline-block group-hover:translate-x-0.5">→</span></span>
                        : "Not yet unlockable"}
                  </button>
                ) : (
                  <p className="mt-7 text-sm text-muted">Connected wallet isn&apos;t the named beneficiary for this vault.</p>
                )}

                {amGuardian && !view.claimable && (
                  <button
                    onClick={attest} disabled={attesting}
          ...
          {ownerValid && view && !view.active && (
            <p className="text-sm text-muted text-center">No active vault switch found for this address.</p>
          )}
            ...
            <div className="relative rounded-3xl border border-accent/40 bg-surface overflow-hidden animate-fade-up">
              ...
              <div className="px-6 sm:px-10 py-10">
                <div className="flex items-center gap-3">
                  <span className="heart-halo text-3xl text-accent animate-heartbeat leading-none">♥</span>
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.18em] font-medium text-accent-deep">Recovered</p>
                    <h3 className="text-xl font-semibold">It&apos;s yours.</h3>
                  </div>
                </div>
                <div className="mt-6">
                  <div className="text-[11px] uppercase tracking-wider text-muted">Recovery phrase / secret</div>
...
{seedPart}
                ...
                {msgPart && (
                  <div className="mt-5">
                    <div className="text-[11px] uppercase tracking-wider text-muted">Message</div>
                    <p className="mt-2 rounded-xl bg-background border border-border-soft p-4 whitespace-pre-wrap break-words text-foreground">{msgPart}</p>
                  </div>
                )}
                <p className="mt-5 text-xs text-muted">
                  Anyone who can see this screen can see your secret. Move it somewhere safe.
                </p>
              </div>
            </div>
          )}
